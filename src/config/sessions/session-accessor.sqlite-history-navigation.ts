...
function navigationCandidatesSql(
  ...
  /* kysely-allow-raw: decoded members only narrow candidates; JavaScript owns exact matching. */
  const match =
    candidate.key === "type"
      ? sql<SqlBool>`member.value IN ('reset', 'compaction', 'custom_message')`
      : candidate.eventIds.length === 1
        ? sql<SqlBool>`instr(member.value, ${candidate.eventIds[0]}) > 0`
        : sql<SqlBool>`EXISTS (SELECT 1 FROM json_each(${JSON.stringify(candidate.eventIds)}) AS requested
  ...
  /* kysely-allow-raw: avoid hydrating unrelated legacy navigation. */
  return sql<SqlBool>`CASE WHEN json_valid(${event}) THEN EXISTS (
