...
export function isAdmissionConflictError(err: unknown): boolean {
  try {
    ...
    return ADMISSION_CONFLICT_FINGERPRINTS.some((fingerprint) => message.includes(fingerprint));
  } catch {
    // Opaque wrapper fields must not interrupt the generic bounded disposition.
    return false;
  }
}
