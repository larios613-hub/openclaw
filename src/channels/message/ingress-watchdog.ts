 ...
 *   watchdog.stop();
 */
export function createDirectUserWatchdog<TPayload, TMetadata extends IngressPriorityMetadata | undefined>(
  config: {
  ...
  const handleStuckMessage = async (stuck: DirectUserStuckMessage): Promise<void> => {
    ...
    if (stuck.ageMs >= errorThresholdMs) {
      log("error", `Direct-user message ${displayId} undispatched for ${Math.floor(stuck.ageMs / 1000)}s; triggering recovery`, {
      ...
      if (currentTime - state.lastRecoveryAt >= recoveryRateLimitMs) {
        ...
        log("warn", `Direct-user watchdog recovery suppressed (rate limit: ${Math.floor(recoveryRateLimitMs / 1000)}s)`, {
          eventId: stuck.eventId,
      ...
      log("warn", `Direct-user message ${displayId} undispatched for ${Math.floor(stuck.ageMs / 1000)}s`, {
        eventId: stuck.eventId,
        channelId: stuck.channelId,
        accountId: stuck.accountId,
        ageMs: stuck.ageMs,
