...
export function collectIngressBacklogHealth(stateDir?: string): IngressBacklogHealth[] {
  ...
  for (const row of quarantineRows) {
    ...
    quarantineByKey.set(key, row.quarantineCount as number);
  }
  ...
  for (const row of pendingRows) {
    ...
    const pendingCount = row.pendingCount as number;
    const oldestReceivedAt = row.oldestReceivedAt as number | null;
  ...
  for (const row of quarantineRows) {
    ...
    const queueName = JSON.stringify([channelId, accountId]);
    const quarantineCount = row.quarantineCount as number;
    const counters = dispatchCountersByQueue.get(queueName);
    results.push({
      channelId,
      accountId,
