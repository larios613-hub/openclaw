    ...
    it("dispatches direct-user messages before normal-priority messages", async () => {
      await withTempState(async (stateDir) => {
        ...
        await queue.enqueue(
          "cron-1",
          { text: "cron job" },
          { laneKey: "cron-lane", receivedAt: clock, metadata: { priority: "normal" } as IngressPriorityMetadata },
        );
        // Then a direct-user message (later receivedAt, different lane).
        await queue.enqueue(
          "dm-1",
          { text: "user message" },
          { laneKey: "dm-lane", receivedAt: clock + 100, metadata: { priority: "direct" } as IngressPriorityMetadata },
        );
    ...
    it("preserves FIFO within direct-user messages", async () => {
      await withTempState(async (stateDir) => {
        ...
        await queue.enqueue(
          "dm-1",
          { text: "first DM" },
          { laneKey: "dm-lane-1", receivedAt: clock, metadata: { priority: "direct" } as IngressPriorityMetadata },
        );
        await queue.enqueue(
          "dm-2",
          { text: "second DM" },
          { laneKey: "dm-lane-2", receivedAt: clock + 100, metadata: { priority: "direct" } as IngressPriorityMetadata },
        );
        await queue.enqueue(
          "cron-1",
          { text: "cron" },
          { laneKey: "cron-lane", receivedAt: clock + 50, metadata: { priority: "normal" } as IngressPriorityMetadata },
        );
    ...
    it("respects prioritySenders config for per-sender priority", async () => {
      await withTempState(async (stateDir) => {
        ...
        await queue.enqueue(
          "cron-1",
          { text: "cron" },
          { laneKey: "cron-lane", receivedAt: clock, metadata: { senderId: "system" } as IngressPriorityMetadata },
        );
        await queue.enqueue(
          "dm-1",
          { text: "from eric" },
          { laneKey: "dm-lane", receivedAt: clock + 100, metadata: { senderId: "eric" } as IngressPriorityMetadata },
        );
    ...
    it("does NOT preempt currently running operations", async () => {
      await withTempState(async (stateDir) => {
        ...
        await queue.enqueue(
          "cron-1",
          { text: "cron" },
          { laneKey: "shared-lane", receivedAt: clock, metadata: { priority: "normal" } as IngressPriorityMetadata },
        );
